...
2. **写入及时**：有价值的信息在当次对话结束前写入对应文件，不遗留在对话里。
3. **最小变更**：每次只做任务要求的事，不随意改动无关文件。
4. **保持轻量**：文件结构简单，需要时才扩展，不提前过度设计。
...
3. **更新长期记忆**：如果对话中出现了应该长期记住的新信息，追加到 `MEMORY.md`。
4. **提交变更**：将上述文件变更提交到仓库，确保记忆持久化。
